...
import Utils.Mod (getInputFile, getExampleInputFile, readInput)
...
traverseWire Wire{currentPoint= curr, points = points} Turn{dir=dir, amount=amount} =
  let newPos = traverseTurn curr dir in
    traverseWire Wire {currentPoint= newPos, points = Set.insert newPos points} Turn{dir=dir, amount=amount -1}
...
getMinDistPart2 (p1, p2) =
  ...
  where distance1 = map getDistancePart2 $ Set.toList $ p1 `Set.intersection` p2
